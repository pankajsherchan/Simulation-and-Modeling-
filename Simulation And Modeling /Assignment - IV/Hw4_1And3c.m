...
result = lsqcurvefit(@(x0, x) ExponentialModel(x0, x) , x0, x, y) %parameter
...
%a -> result(1)      
% b -> result(2)
fplot(result(1)*exp(result(2)*xx))

xlabel('x')
ylabel('y')
legend('data', 'lsqcurvefit for y = a*e^(bx)')

xlim ([0 0.15])
ylim([0 0.005])

fittedValues = result(1)*exp(result(2)*x);
